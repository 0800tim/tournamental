...
- `docs/VTorn PITCH.md` (duplicate of root `VTorn Pitch.md` — skip)
...
```
You are the VTorn project bootstrap orchestrator on Tim's dev server. The
...
1. Orient and verify the copy.
   ...
   - Confirm that .DS_Store, .git/, docs/VTorn PITCH.md, and
   ...
   - Read REVIEW.md fully. Read CLAUDE.md fully. Read AGENT-PROMPTS.md
     top to bottom. Skim README.md and "VTorn Pitch.md".
...
2. Initialize git and push to a new private GitHub repo.
   ...
   - Commit with `git commit -s` and the following message body:
     ...
     chore: import VTorn design pack v0.1.1
   ...
   - Create the remote and push in one shot:
       gh repo create 0800tim/vtorn --private --source=. --remote=origin \
         --description "VTorn — verified tournament prediction game + 3D match renderer (vtorn.com)" \
         --push
