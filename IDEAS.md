...
- [2026-05-11, share-landing] Hoist `bracketToCascadeInput` from `apps/web/lib/bracket/cascade-bridge.ts` + `apps/game/src/routes/bracket-cascade-summary.ts` into `@tournamental/bracket-engine` so both surfaces share one helper. Currently near-duplicated. (open)
...
- [2026-06-04, CVE sweep] Bump `apps/marketing` to astro 5.x (currently pinned to ^4.16.19). Astro 5 tightens the `z.union([image(), z.string()])` schema and the union no longer falls through cleanly for `/public/blog/*.jpg` heroImage strings: build fails with ImageNotFound on every blog post. Fix is either (a) reorder the union or (b) move hero refs to a dedicated `heroImageUrl` string field. Tracking for a deliberate migration once the blog content set is locked. Until then four astro CVEs remain unpatched (1 high reflected-XSS GHSA-7g5w-7gv5-mjj9, 3 moderates). (open)
- [2026-06-04, CVE sweep] Bump `apps/web` from next-intl 3.26.5 to ^4.9.2. v4 changes the navigation/routing API (Link, useRouter) and several locale-detection internals; sweeping refactor across i18n call-sites. Two moderate CVEs (open-redirect, prototype pollution via experimental.messages.precompile) remain on 3.x. (open)
- [2026-06-04, CVE sweep] Bump `apps/web` to next 15.x once App Router 14 route conventions are audited. 15.5.16 patches eleven highs/moderates (DoS, SSRF via WebSocket, image-optimizer cache-key, App Router XSS); none are exploitable on our current surface but the audit count drops sharply. (open)
- [2026-06-04, CVE sweep] Bump `vitest` from 2.x to ^4.1.0 across all packages — clears the critical advisory (vitest UI server arbitrary file read/execute). Dev-only impact, contained, but worth doing on the next refactor wave. (open)
