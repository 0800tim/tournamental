...
> The rulebook. Ten game modes, the scoring formula that makes them all interoperable, the multipliers, the streaks, the confidence mechanic, and the personality leaderboards. Brand framing is in [doc 15](15-vtorn-brand-and-positioning.md); the engine that runs all of this lives in `apps/game-service` per agent J ([doc 09](09-agent-task-breakdown.md)).
...
- **Perfect Tournament: +100,000 bonus** (or a sponsor-provided physical prize where available; see [doc 15](15-vtorn-brand-and-positioning.md)).
...
1. **VTorn Crowd** — aggregate of all VTorn user predictions on that outcome at lock time. Median, not mean (robust to whales).
