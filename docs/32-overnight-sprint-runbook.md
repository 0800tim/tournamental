...
| --- | --- | --- |
| Marketing site | https://www.vtourn.com | Nav has "Play World Cup 2026 →" CTA |
| WC apex hype landing | https://2026wc.vtourn.com/ | Countdown, 48-team grid, syndicate signup form |
| Bracket builder | https://2026wc.vtourn.com/world-cup-2026 | All groups + cascade through Final, live odds chips on hover |
| Replay (2022 final) | https://app.vtourn.com/match/fifa-wc-2022-final-arg-fra-2022-12-18 | Plays through, scrubber works |
| Admin (when up) | https://admin.vtourn.com | Magic-link login, allowlisted emails |
| Live odds API (when up) | https://odds.vtourn.com/v1/odds/snapshot | JSON dump of all current probabilities |
| Auth API (when up) | https://auth.vtourn.com | Phone-OTP request endpoint |
| Telegram bot (when up) | https://t.me/VTournBot (TBC) | /start onboards |
...
| #46 | WC2026 hype landing on `2026wc.vtourn.com/` apex | 10-section marketing landing |
...
| #42 | Host-aware redirect (replay → app.vtourn.com) | Cleans 2026wc subdomain to be 100% WC |
| #41 | Brand audit VTorn → VTourn | 340 replacements, 62 files |
...
```
www.vtourn.com / vtourn.com           → apps/marketing (Astro)         → :4321
app.vtourn.com                        → apps/web      (Next 14)        → :3300
api.vtourn.com                        → apps/api      (Fastify)        → :3310
2026wc.vtourn.com / wc2026.vtourn.com → apps/web      (host-aware)     → :3300
                                          - apex `/`        → /world-cup-2026/landing (hype)
                                          - /world-cup-2026 → bracket builder
                                          - /match/*        → 308 → app.vtourn.com/match/*
admin.vtourn.com   (when up)          → apps/admin       (Next 14)     → :3340
auth.vtourn.com    (when up)          → apps/auth-sms    (Fastify)     → :3330
bot.vtourn.com     (when up)          → apps/tournament-bot (grammy)   → :3350
odds.vtourn.com    (when up)          → apps/odds-ingest (Fastify)     → :3320
stream.vtourn.com                     → AR-FR stream producer (WS)     → :4001
```
...
2. **Polymarket affiliate registration** — KYC for VTourn Holdings; first-deposit attribution callback verification.
...
6. **WhatsApp Baileys pairing** — when `apps/auth-sms` is up, scan the QR shown at `https://auth.vtourn.com/v1/auth/whatsapp/pairing-qr` once with WhatsApp Web on Tim's account.
...
- [ ] `https://www.vtourn.com/` — header has new CTA
- [ ] `https://2026wc.vtourn.com/` — countdown ticking, all 48 flags rendering
- [ ] `https://2026wc.vtourn.com/world-cup-2026` — pick a few group matches, check standings update; switch to Knockouts; click R32 → R16 → QF → SF → Final all populate
- [ ] Hover any group match → odds chip appears with W/D/L percentages
- [ ] `https://app.vtourn.com/match/fifa-wc-2022-final-arg-fra-2022-12-18` — replay plays, timeline scrubs
- [ ] `https://admin.vtourn.com` — magic link works
- [ ] `https://odds.vtourn.com/v1/odds/snapshot` — returns JSON with at least mock data
