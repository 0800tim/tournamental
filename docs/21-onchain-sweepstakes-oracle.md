...
- **Not the custodian.** Funds live in the pool contract on-chain, controlled by the contract logic, never touchable by Tournamental.
...
```
              ...
              └──────────────┬──────────────┘
                             ...
                             ▼
              ┌─────────────────────────────┐
              │  TournamentalOracle contract       │   ◄── Tournamental Foundation publishes
...
1. Tournament ends; Tournamental publishes results to `TournamentalOracle` (typically a single transaction batch covering an entire tournament's worth of match outcomes; ~$5–$20 on Polygon).
...
End to end, Tournamental never holds funds. Members never need to trust the creator. Creator never needs to chase members for entry fees. Result is published once, on-chain, immutable.

## TournamentalOracle in detail
...
For pool members, USDC winnings are typically taxable income in their jurisdiction. Tournamental does not issue tax forms (we don't see the transactions; the user's wallet does). Pool participants are responsible for their own tax compliance.
...
contract Pool {
    ...
    ITournamentalOracle public immutable oracle;
...
contract TournamentalOracle {
    address public multisig;          // 4-of-7 governance multisig
    mapping(bytes32 => ITournamentalOracle.MatchResult) private _results;

    function setResult(ITournamentalOracle.MatchResult calldata r) external {
    ...
    function correctResult(bytes32 matchId, ITournamentalOracle.MatchResult calldata r) external {
        require(msg.sender == multisig, "unauthorized");
...
- [ ] After `TournamentalOracle.setResult(...)` for all the pool's matches, anyone can call `pool.finalize()` and the contract correctly distributes USDC per the prize split.
