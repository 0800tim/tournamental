...
| **Facebook** | Medium (+10) | Yes (mutual friends only — Meta's API restriction) | Yes | name, email, profile photo, friend count, mutual friends with other VTorn users | Friend graph is the killer feature here despite Meta's API decline. |
...
| **GitHub** | Medium (+8) for everyone, +20 for VTorn contributors | No | Yes | name, email, public profile, contribution graph | Proves real long-term human activity for devs. |
...
- **Contacts API.** With explicit OS-level permission, the app reads the user's address book and looks up matches in VTorn (by phone number hash or email hash — never raw values uploaded). Friend candidates surface in the app: "8 of your contacts already play VTorn. Add them?"
...
3. Sends the hashes to VTorn's API.
4. Server matches the hashes against existing user phone-number-hashes.
5. Returns the *user IDs* of matching VTorn users.
6. App displays "8 of your contacts play VTorn" with their VTorn display names + avatars.

The salt is user-specific so VTorn cannot cross-reference contacts across the user base. The hashes are stored in Redis with a 7-day TTL; recomputed on next contact-sync. Privacy reviewers can audit this; users can disable contact sync at any time.
...
A user shares a WhatsApp message containing a one-time code we DM them to a designated VTorn WhatsApp Business number. We confirm their phone number is theirs and that WhatsApp itself (Meta) has previously verified that number. Cost: ~$0.005 per verification message (template message rate). Worth it for a high-quality humanness signal on first-time users.

### B — In-WhatsApp friend invites and group leaderboards

Native iOS share-intent: the user picks "Invite via WhatsApp" → the OS hands a pre-filled WhatsApp message to their friends → friends tap and land on a VTorn invite page. Conversion rate on this flow is dramatically higher than email or Telegram invite for any user with WhatsApp installed.

For group chats, WhatsApp Business API lets us mirror the Telegram group leaderboard pattern from [doc 13](13-telegram-bot-and-auth.md): a user invites the VTorn bot into a WhatsApp group, the group becomes a private leaderboard, the bot posts results. Cost is non-zero (Meta's WhatsApp Business API is metered) but the humanness + viral mechanics justify it for engaged groups.
...
- **Mutual friend pairs that are *also* connected on a non-VTorn social network.** A friendship that exists on VTorn AND on Facebook AND on WhatsApp is much harder to fake than one that exists only on VTorn.
