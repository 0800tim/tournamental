...
const realExec = (cmd: string, args: string[]) =>
  new Promise<{ code: number; stdout: string; stderr: string }>((resolve) => {
    const c = spawn(cmd, args, { stdio: 'pipe', shell: false });
    let stdout = '';
    let stderr = '';
...
export async function reloadOrRestart(opts: Pm2Options): Promise<Pm2Result> {
  ...
  log(`[pm2] reload ${opts.name}`);
  const r = await exec('pm2', ['reload', opts.name, '--update-env']);
  if (r.code === 0) {
    return {
      action: 'reload',
  ...
  const restart = await exec('pm2', ['restart', opts.name, '--update-env']);
  if (restart.code === 0) {
    return {
      action: 'restart',
      durationMs: Date.now() - t0,
