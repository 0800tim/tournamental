 ...
 *
 * Visual: TVNZ-app inspired flag-bg card. Each match cell is split
...
function buildIcs(match: UpcomingMatch): string {
  ...
  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Tournamental//WC2026//EN",
    "BEGIN:VEVENT",
    `UID:tournamental-wc2026-${match.match_number}@tournamental.com`,
    `DTSTAMP:${fmt(new Date())}`,
    `DTSTART:${fmt(start)}`,
    `DTEND:${fmt(end)}`,
    `SUMMARY:${match.home.name} vs ${match.away.name}, World Cup 2026`,
