...
const blog = defineCollection({
  ...
  schema: z.object({
    ...
    // Hero images are referenced as static URLs under `/public/blog/`.
    // Astro 5 tightened `image()` so `z.union([image(), z.string()])`
    // treats public paths as local assets and fails the build with
    // ImageNotFound. We accept plain URL strings here and let the
    // template render them via a plain `<img>` tag; the public-asset
    // CDN handles caching. Astro's image optimisation pipeline isn't
    // load-bearing for marketing hero images (not LCP-critical).
...
const press = defineCollection({
  ...
  schema: z.object({
    ...
    audience: z
      ...
      .default(["general"]),
    // See blog collection comment: Astro 5 forces us to drop `image()`
    // here because press hero images live under `/public/press/` as
    // static URLs.
    heroImage: z.string().optional(),
    heroImageAlt: z.string().optional(),
...
const engineering = defineCollection({
  ...
  schema: z.object({
    ...
    tags: z.array(z.string()).default([]),
    // See blog collection comment: Astro 5 forces us to drop `image()`
    // because engineering hero images live under
    // `/public/blog/engineering/` as static URLs.
    heroImage: z.string().optional(),
    heroImageAlt: z.string().optional(),
    heroImageCredit: z.string().optional(),
    draft: z.boolean().default(false),
